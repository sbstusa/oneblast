...
Then, import an additional file containing last name and first name and email
...
communication. So the problem is that we do not know which treatment arm some
of these people belong to. [CHECK THIS!]
...
## Now create a dataset with unique rows for email addresses --- one row per address, but up to 24 columns of names associated with that address.
...
length(setdiff(row.names(rand),row.names(flnamesFinal)))

randPlusNames<-merge(rand,flnamesFinal,all.x=TRUE)
str(randPlusNames)
head(randPlusNames)
...
sum(subscriptions$Email.Address %in% randPlusNames$Email.Address)

finaldat <- merge(randPlusNames,subscriptions,by="Email.Address", all=TRUE,sort=FALSE)

finaldat2 <- merge(designdata,subscriptiondata,all=TRUE,sort=FALSE)

stopifnot(nrow(finaldat)==nrow(finaldat2))
