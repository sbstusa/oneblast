...
```{r overalltab, results="asis"}
taball<-table.maker(wrkdat$treatmentF,xnm="treatmentF",ynm="subscribed")
...
distribution of unmatched cases by batch.
...
extremeprops<-function(y){
	thefmla<-as.formula(paste(y,"~treatmentF*(",paste(colnames(Bmd),collapse="+"),")-1"))
	thelm<-lm(thefmla,data=wrkdat)
	theprops<-coef(thelm)[grep(":",grep("treatmentF",names(coef(thelm)),value=TRUE),invert=TRUE,value=TRUE)]
	theCI<-confint.HC(thelm,parm=names(theprops),thevcov=vcovHC(thelm,type="HC2"))
	thepval <- coeftest(thelm,vcov=vcovHC(thelm,type="HC2"))[names(theprops),"Pr(>|t|)"]
	if(length(theprops)>1){
		return(cbind(theprops,thepval,theCI))
	} else {
		return(c(theprops,thepval,theCI))
	}
}
