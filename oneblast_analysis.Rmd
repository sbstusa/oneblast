...
prop_fn<-function(x){
    ...
    c(proptest$estimaate,proptest$conf.int)
}
...
theprops$Treatment<-sort(unique(wrkdat$treatment))
# Plot with ggplot2
library(ggplot2)
...
table.maker<-function(x){
    ...
    cmh_approx.stat<-sprintf("%.4f",statistic(cmh_bdf_approx))
    cmh_approx.pval<-sprintf("%.4f",pvalue(cmh_bdf_approx))
...
# Assess sensitivity of the results to different ways of distributing
...
# Functio to randomly reassign unmatched cases by batch day
...
Here are the differences in proportions subscribing after weighting by size of batch.
...
```{r}
# 
# # Playing around with getting pairwise comparisons from lm
# 
# # Helper function
# 
# table_maker<-function(x=wrkdat$treatmentF){
#     k<-length(levels(x))
#     levs<-levels(x)
#     # Comparisons
#     comp<-choose(k,2)
#     # Matrix for comparisons
#     out<-matrix(NA,ncol=4,nrow=comp)
#     rows<-NULL
#     # Row names
#     for(i in 1:(k-1)){
#         for(j in (i+1):k){
#             rows<-c(rows,paste(levs[j],levs[i],sep="-"))
#         }
#     }
#     rownames(out)<-rows
#     colnames(out)<-c("diff","lwr","upr","p value")
#     return(out)
# }
# table_maker()
# pairwise_lm<-function(x=wrkdat$treatmentF, adj="holm"){
...
#     for(i in 2:k){
#     # Fit model
#         fm<-lm(I(subscribed-sBatchAve)~relevel(treatmentF,ref=i-1),wrkdat)
#     # Differences
#     coefs<-c(coefs,coeftest(fm)[i:k,1])
#     # P values
#     pvals<-c(pvals,p.adjust(coeftest(fm)[i:k,4],adj,n = choose(k,2)))
#     # Confidence intervals
#     ci<-rbind(ci,confint(fm)[i:k,])
# 
#     }
#     out[,1]<-coefs
#     out[,4]<-pvals
...
wrkdat$subscribe<-ifelse(wrkdat$subscribed==1,"Subscribed","Not Subscribed")
...
- In treatment conditions where the block 10 is absent (E,F) rather than present (A,B,C,D)
