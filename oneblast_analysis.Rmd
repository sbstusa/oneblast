...
There is a lot of evidence against the idea that all six emails had the same effect and, despite the rareness of the outcome, the substantive interpretation of the test is the same.
...
When block sizes vary and especially when the proportion treated per block varies greatly, one can substantially increase precision by estimating using what are known as harmonic weights. In this case, with more or less equal sized blocks (40 out of 41 are equal), and more or less equal proportions, we might also use the simple block size weighting.
...
Lin (2011,2013) says that block size weighting can be done via a least squares linear model. Here, we use a small subset of the data to show that this is so.
...
N <- nrow(wrkdatsmall)
n_b<-table(wrkdatsmall$batchday2) ## assuming same order as tau_b
prop_b<-n_b/N
tau_b<-sapply(split(wrkdatsmall,wrkdatsmall$batchday2F),function(dat){ 
				  sapply(levels(dat$treatmentF),function(i){
							  mean(dat$subscribed[dat$treatmentF==i]) })
})
...
## Verify by hand.
with(wrkdatsmall, sum(subscribed[treatmentF=="A" & batchday2F=="1"])/sum( treatmentF=="A" & batchday2F=="1" ) )
with(wrkdatsmall, sum(subscribed[treatmentF=="D" & batchday2F=="2"])/sum( treatmentF=="D" & batchday2F=="2" ) )

## Since props_b are equal in the test case 
mean(tau_b[1,])


B <- model.matrix(~batchday2F+0,data=wrkdatsmall)
Bmd <- sapply(1:ncol(B),function(i){ B[,i] - prop_b[i] }) ## subtraction is columnwise
...
## THe mean of the batch factor indicators is 0 by construction
...
## Remove any systematic batch mean effects from the outcome
wrkdat$sBatchAve<-ave(wrkdat$subscribed,wrkdat$batchday2F)
## wrkdat$subscribedMD<-with(wrkdat,unsplit(lapply(split(subscribed,batchday2F),function(x){ x - mean(x) }),batchday2F))
wrkdat$subscribedMD<-wrkdat$subscribed - wrkdat$sBatchAve
stopifnot(all(round(with(wrkdat,tapply(subscribedMD,batchday2,mean)),10)==0))

## Notice that a positive number is a 1 and a negative is a 0
table(wrkdat$subscribedMD>0,wrkdat$subscribed)
